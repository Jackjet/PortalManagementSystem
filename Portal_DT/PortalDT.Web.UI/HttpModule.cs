...
using System.Xml;
using PortalDT.Common;

namespace PortalDT.Web.UI
{
    /// <summary>
    /// DTcms的HttpModule类
    /// </summary>
    public class HttpModule : System.Web.IHttpModule
    {
        ...
        private void ReUrl_BeginRequest(object sender, EventArgs e)
        {
            ...
            if (requestPath == "/music.html")//music页面的请求
            {
                string url = "1";
            }
            ...
            string sitePath = GetSitePath(siteConfig.webpath, requestPath, requestDomain); //获取当前站点目录：main
            string requestPage = CutStringPath(siteConfig.webpath, sitePath, requestPath); //截取除安装、站点目录部分

            ArrayList urlList = SiteUrls.GetUrls().Urls;//********url.config文件下的配置项
            //检查网站重写状态0表示不开启重写、1开启重写、2生成静态
            if (siteConfig.staticstatus == 0)
            {
                #region 站点不开启重写处理方法===========================
                //遍历URL字典，匹配URL页面部分
                foreach (Model.url_rewrite model in urlList)
                {
                    if (model.page == "music.aspx")//******不开启重启,如果是aspx页面则会直接去找aspx目录下去寻找对应的页面
                    {
                        string url = "1";
                    }
                    //查找到与页面部分匹配的节点
            ...
            else
            {
                ...
                foreach (Model.url_rewrite model in urlList)
                {
                    if (requestPath == "/music.html")
                    {
                        if (model.page == "music.aspx")
                        {
                            string url = "1";
                        }
                    }
                    //******如果没有重写表达式则不需要重写,根据url.config配置的匹配项，找到对应的page
                        ...
                        Utils.GetUrlExtension(model.page, siteConfig.staticextension) == requestPath.Substring(requestPath.LastIndexOf("/") + 1))
                    {
                        ...
                        {
                            string rewriteUrl = string.Format("{0}{1}/{2}/{3}",
                    ...
                    foreach (Model.url_rewrite_item item in model.url_rewrite_items)
                    {
                            ...
                            || (model.page == "index.aspx" && Regex.IsMatch(requestPage, string.Format("^/{0}$", item.pattern), RegexOptions.None | RegexOptions.IgnoreCase)))
                        {
                            ...
                            else //其它
                            {
                                ...
                                string rewriteUrl = string.Format("{0}{1}/{2}/{3}",
                                    siteConfig.webpath, DTKeys.DIRECTORY_REWRITE_ASPX, sitePath, model.page);
                                context.RewritePath(rewriteUrl, string.Empty, queryString);
                                return;
                            }
        ...
        private bool IsDirExist(string cacheKey, string webPath, string dirPath, string requestPath)
        {
            ...
            {
                //如果requestFirstPath==list中的一种，则true
                return true;
            }
            return false;
        }
