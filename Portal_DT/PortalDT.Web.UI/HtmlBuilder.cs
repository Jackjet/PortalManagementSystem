...
namespace PortalDT.Web.UI
{
    public class HtmlBuilder
    {
        ...
        public void getpublishsite(HttpContext context)
        {
            ...
            sbjson.Append("]");
            //sbjson=tools/admin_ajax.ashx?action=get_builder_html&lang=main&html_filename=&indexy=&aspx_filename=aspx/main/music.aspx&catalogue=html/main/music.html
        ...
        {
            if (File.Exists(Utils.GetMapPath(config.webpath + aspx_filename.Substring(0, aspx_filename.IndexOf(".aspx") + 5))))// aspx/mian/music.aspx
            {

                string urlPath = config.webpath + aspx_filename.Replace("^", "&"); //文件相对路径 aspx_filename=aspx/main/music.aspx
                string htmlPath = config.webpath + catalogue; //保存相对路径 ,catalogue=html/main/music.html
