<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="AdvertisingManager.aspx.cs" Inherits="PMSWeb.Admin.AdvertisingManager" %>
...
<head runat="server">
    ...
    <script id="item_advert" type="text/x-jquery-tmpl">
        <tr>
            ...
            <td>${Description}</td>
            <%--'s${Id}'--%>
            <td><input type="text"  value="${SortId}" id="${Id}" style="width:50px;height:24px;border:1px solid #dcdcdc;border-radius:2px;"/> <input type="button" value="保存"  onclick="UpSortId('${Id}', '${Id}')" style="min-width:50px;height:28px;background:#007cdb;color:#fff;border-radius:2px;"/></td>
            <td>${DateTimeConvert(CreateTime)}</td>
            <td>
                <a href="javascript:;" onclick="UpOrDown('${Id}',${SortId},'up')"><i class="icon icon-arrow-up"></i>上移</a>
                <a href="javascript:;" onclick="UpOrDown('${Id}',${SortId},'down')"><i class="icon icon-arrow-down"></i>下移</a>
                <a href="javascript:;" onclick="SetAdvertisingTop('${Id}','${MenuId}',1)"><i class="icon icon-arrow-up"></i>置顶</a>
            </td>
        </tr>
    </script>
    <style>
...
<body>
            ...
            <div class="course_manage bordshadrad">
                ...
                <div class="newcourse_select clearfix">
                    ...
                    <div class="clearfix fl course_select">
                        <label for="">类型：</label>
                        <select id="menuType"  onchange="query()" class="select">
                            
                        </select>
                        <%--<label for="" class="ml10">显示条数：</label>
                        <input type="text"  value=" " class="tiao fl" />
                        <label for="" class="ml10">时间或者点击量展示设置：</label>
                        <div class="fl">
                            <input type="radio" name="name" value="" class="fl radio"/>
                            <label for="" class="ml10">时间</label>  
                            <input type="radio" name="name" value="" class="fl radio" />
                            <label for="" class="ml10">点击量</label>
                        </div>
                        <input type="button" name="name" value="确定" class="Btn"/>--%>
                    </div>
                </div>
                <div id="tabA">
                    <div class="wrap">
    ...
    <script type="text/javascript">
        ...
        function UpOrDown(id, sid, status) {
            $.ajax({
                url: "/Common.ashx",
                type: "post",
                async: false,
                dataType: "json",
                data: { PageName: "DYYX/AdvertisingHandler.ashx", Func: "UpOrDownAdvert", Id: id, MenuId: $("#menuType").val(), SortId: sid, upOrdown: status,isPush:1 },
                success: function (json) {
                    if (json.result.errMsg.toString() == "success") {
                        query();
                    } else if (json.result.errMsg.toString() == "max") {
                        layer.msg("当前已是最大值！");
                    } else if (json.result.errMsg.toString() == "min") {
                        layer.msg("当前已是最小值！");
                    }
                    else { layer.msg('操作失败！'); }
                },
        ...
        function SetAdvertisingTop(id,menuid,top) {
            $.ajax({
                url: "/Common.ashx",
                type: "post",
                async: false,
                dataType: "json",
                data: { PageName: "DYYX/AdvertisingHandler.ashx", Func: "SetAdvertisingTop", Id: id, IsTop: top },
                success: function (json) {
        ...
        function UpSortId(id, inputid) {

            var sort =  $("#" + inputid).val() ;
            ...
            $.ajax({
                ...
                async: false,
                dataType: "json",
                data: { PageName: "DYYX/AdvertisingHandler.ashx", Func: "UpdateAdvertising", Id: id, Sort: sort },
                success: function (json) {
                    if (json.result.errNum.toString() == "0") {
                        layer.msg("修改成功！");
                        query();
